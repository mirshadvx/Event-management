...
class SocialMediaLink(models.Model):
    user = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="social_media_links"
    )
...
class UserSettings(models.Model):
    ...
    marketing_emails = models.BooleanField(
        default=False, help_text="Receive promotional emails"
    )
...
class WalletTransaction(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.transaction_type} of ₹{self.amount} for {self.wallet.user.username}"
        )
...
class Booking(models.Model):
    ...
    event = models.ForeignKey(
        "event.Event", on_delete=models.CASCADE, related_name="bookings"
    )
    ...
    ticket_purchases = models.ManyToManyField(
        "event.TicketPurchase", related_name="booking"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class TicketRefund(models.Model):
    wallet_transaction = models.ForeignKey(
        WalletTransaction, on_delete=models.CASCADE, related_name="refund_details"
    )
    ticket_type = models.CharField(max_length=50)
    quantity = models.PositiveIntegerField()
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    event = models.ForeignKey(
        "event.Event", on_delete=models.SET_NULL, null=True, related_name="refunds"
    )
    booking = models.ForeignKey(
        "Booking", on_delete=models.SET_NULL, null=True, related_name="refunds"
    )
