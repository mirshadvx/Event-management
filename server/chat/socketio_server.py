                ...
                try:
                    ...
                    if room_name and notification_data and main_event_loop:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Error emitting notification to room {room_name}: {str(emit_error)}"
                            )
                    else:
                        if not main_event_loop:
                            logger.warning(
                                "Main event loop not available, cannot emit notification"
                            )
                        else:
...
async def connect(sid, environ, auth):
    try:
        ...
        if redis_listener_task is None or (
            redis_listener_task and not redis_listener_task.is_alive()
        ):
            try:
...
async def join_chat(sid, data):
    try:
        ...
        if chat_type == "personal":
            is_participant = await sync_to_async(is_conversation_participant)(
                chat_id, user.id
            )
...
async def leave_chat(sid, data):
    ...
    if room_name:
        ...
        await sio.save_session(
            sid, {**session, "chat_type": None, "chat_id": None, "room_name": None}
        )
...
async def send_message(sid, data):
    try:
        ...
        if chat_type == "personal":
            ...
            saved_message = await sync_to_async(save_group_message)(
...
async def send_image(sid, data):
    try:
        ...
        if chat_type == "personal":
            saved_message = await sync_to_async(save_image_message)(
                chat_id, user.id, image_data
            )
        elif chat_type == "group":
            saved_message = await sync_to_async(save_group_image_message)(
                chat_id, user.id, image_data
            )
        else:
            await sio.emit("error", {"message": "Invalid chat type"}, room=sid)
...
async def mark_read(sid, data):
    try:
        ...
        if chat_type == "personal":
            ...
            await sync_to_async(mark_group_message_as_read)(
                chat_id, user.id, message_id
            )
...
async def join_notifications(sid, data):
    try:
        ...
        if str(user.id) != str(user_id):
            logger.warning(
                f"User ID mismatch: session user {user.id} != requested {user_id}"
            )
            await sio.emit("error", {"message": "Unauthorized"}, room=sid)
            return
...
def mark_group_message_as_read(chat_id, user_id, message_id):
    try:
        user = Profile.objects.get(id=user_id)
        message = GroupMessage.objects.get(id=message_id, conversation_id=chat_id)
        if message.sender != user and not message.read_by.filter(id=user.id).exists():
            message.read_by.add(user)
...
async def send_notification_to_user(user_id, notification_data):
    try:
        ...
        logger.info(
            f"Emitting notification to room: {room_name}, data: {notification_data}"
        )
