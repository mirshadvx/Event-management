...
urlpatterns = [
    ...
    path(
        "group-conversations/<int:pk>/",
        GroupConversationDetailView.as_view(),
        name="group-conversation-detail",
    ),
    path(
        "group-conversations/<int:group_id>/messages/",
        GroupMessageListCreateView.as_view(),
        name="group-message-list-create",
    ),
    path(
        "group-conversations/<int:group_id>/messages/<int:pk>/",
        GroupMessageRetrieveDestroyView.as_view(),
        name="group-message-detail",
    ),
    path(
        "group-messages/<int:message_id>/read/",
        MarkGroupMessageAsReadView.as_view(),
        name="mark-group-message-read",
    ),
    ...
    path(
        "chatInfo/<int:chat_id>/<str:chat_type>/", ChatInfo.as_view(), name="chatInfo"
    ),
]
