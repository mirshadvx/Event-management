...
class GroupConversationListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        user = self.request.user
        return GroupConversation.objects.filter(participants=user).order_by(
            "-updated_at"
        )
...
class GroupMessageRetrieveDestroyView(generics.RetrieveDestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if (
            ...
            and instance.conversation.admin != request.user
        ):
            return Response(
                {"error": "Only the sender or group admin can delete this message"},
                status=status.HTTP_403_FORBIDDEN,
            )
...
class ChatInfo(views.APIView):
    ...
    def get(self, request, chat_id, chat_type):
        if chat_type == "personal":
            ...
            return Response(
                {"error": "Invalid chat type"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class NotificationList(views.APIView):
    ...
    def delete(self, request):
        ...
        return Response(
            {"message": "all notifications deleted"}, status=status.HTTP_200_OK
        )
