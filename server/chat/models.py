...
class ConversationManager(models.Manager):
    def get_queryset(self):
        return (
            ...
            .prefetch_related(
                Prefetch(
                    "participants", queryset=Profile.objects.only("id", "username")
                )
            )
        )
...
class GroupConversation(models.Model):
    ...
    admin = models.ForeignKey(
        Profile, on_delete=models.SET_NULL, null=True, related_name="admin_groups"
    )
    event = models.OneToOneField(
        "event.Event", on_delete=models.CASCADE, related_name="group_chat", null=True
    )
...
class GroupMessage(models.Model):
    ...
    read_by = models.ManyToManyField(
        Profile, related_name="read_group_messages", blank=True
    )
    is_image = models.BooleanField(default=False)
