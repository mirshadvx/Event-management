...
class RevenueDistributionFilter(django_filters.FilterSet):
    ...
    event_type = django_filters.ChoiceFilter(
        field_name="event__event_type", choices=Event.EVENT_TYPE_CHOICES
    )
    ...
    def filter_search(self, queryset, name, value):
        return queryset.filter(
            ...
            | Q(event__organizer__username__icontains=value)
        )
    ...
    def filter_date_range(self, queryset, name, value):
        ...
        if value == "today":
            ...
            return queryset.filter(
                distributed_at__year=now.year, distributed_at__month=now.month
            )
        elif value == "year":
...
class BookingFilterHistory(django_filters.FilterSet):
    ...
    payment_method = django_filters.ChoiceFilter(
        field_name="payment_method", choices=Booking.PAYMENT_METHOD_CHOICES
    )
    ...
    def filter_search(self, queryset, name, value):
        return queryset.filter(
            Q(event__event_title__icontains=value) | Q(user__username__icontains=value)
        )
...
class EventFilter(django_filters.FilterSet):
    ...
    event_type = django_filters.CharFilter(
        field_name="event_type", lookup_expr="iexact"
    )
    date_range = django_filters.CharFilter(method="filter_date_range")
    start_date = django_filters.DateFilter(method="filter_custom_date")
    end_date = django_filters.DateFilter(method="filter_custom_date")
...
class DashBoardFilter(django_filters.FilterSet):
    event_type = django_filters.MultipleChoiceFilter(
        field_name="event_type", choices=Event.EVENT_TYPE_CHOICES
    )
    date_range = django_filters.CharFilter(method="filter_date_range")
    start_date = django_filters.DateFilter(field_name="created_at", lookup_expr="gte")
    end_date = django_filters.DateFilter(field_name="created_at", lookup_expr="lte")
