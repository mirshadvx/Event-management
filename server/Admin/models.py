...
class OrganizerRequest(models.Model):
    user = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="organizer_requests"
    )
    ...
    admin_notes = models.TextField(
        blank=True, help_text="Notes from admin on approval/rejection"
    )
...
class Coupon(models.Model):
    ...
    discount_type = models.CharField(
        choices=[("fixed", "Fixed"), ("percentage", "Percentage")], max_length=20
    )
    discount_value = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Discount amount or percentage"
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Is the coupon currently active?"
    )
    usage_limit = models.PositiveIntegerField(
        default=1, help_text="Total usage limit across all users"
    )
    ...
    used_by = models.ManyToManyField(
        ...
        help_text="Users who have used this coupon",
    )
...
class UserBadge(models.Model):
    ...
    user = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="user_badges"
    )
...
class RefundTicket(models.Model):
    ...
    ticket_purchase = models.OneToOneField(
        TicketPurchase, on_delete=models.CASCADE, related_name="refund"
    )
...
class RevenueDistribution(models.Model):
    event = models.OneToOneField(
        Event, on_delete=models.CASCADE, related_name="revenue_distribution"
    )
...
class UserSubscription(models.Model):
    user = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="user_subscription"
    )
...
class SubscriptionTransaction(models.Model):
    subscription = models.ForeignKey(
        UserSubscription, on_delete=models.CASCADE, related_name="transactions"
    )
    amount = models.DecimalField(max_digits=10, decimal_places=2)
