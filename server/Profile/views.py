    ...
    def post(self, request):
        try:
            ...
            if username is None or follow_action is None:
                return Response(
                    {"error": "Username and follow status are required."},
            ...
            has_premium = (
                subscription is not None and subscription.plan.name == "premium"
            )
            ...
            if follow_action is True:
                ...
                follow = Follow.objects.create(
                    follower=request.user, followed=followed_user, status=follow_status
                )
                ...
                return Response(
                    {"message": message, "status": follow_status},
...
class FollowRequestList(APIView):
    ...
    def delete(self, request):
        ...
        return Response(
            {"message": "All follow requests cleared."}, status=status.HTTP_200_OK
        )
...
class FollowRequestDetail(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            follow_request = Follow.objects.get(
                pk=pk, followed=request.user, status="pending"
            )
            if action == "accept":
    ...
    def accept_follow_request(self, request, follow_request):
        ...
        existing_convo = (
            Conversation.objects.filter(participants=follower_profile)
            .filter(participants=followed_profile)
            .distinct()
        )
        ...
        return Response(
            {"message": "request accepted successfully"}, status=status.HTTP_200_OK
        )
    ...
    def reject_follow_request(self, follow_request):
        ...
        send_user_notification.delay(follower_profile.id, message)
        return Response(
            {"message": "request reject successfully"}, status=status.HTTP_200_OK
        )
...
class ReviewAPIView(APIView):
    ...
    def get(self, request, event_id):
        try:
            ...
            return Response(
                {"error": "No review found for this event"},
