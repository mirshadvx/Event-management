...
async def websocket_handler(scope, receive, send):
    ...
    try:
        if path.startswith("/ws/webrtc/"):
            ...
            logger.info(
                f"[ASGI] Event routing patterns: {event_routing.websocket_urlpatterns}"
            )
            ...
            try:
                ...
                logger.error(
                    f"[ASGI] Error in middleware/routing: {str(middleware_error)}",
                    exc_info=True,
                )
            ...
            or path.startswith("/ws/notifications/")
            or path.startswith("/ws/organizer/")
        ):
            logger.info(
                f"[ASGI] Routing chat/notification/organizer connection to Django Channels"
            )
            middleware_stack = TokenAuthMiddleware(
                URLRouter(
