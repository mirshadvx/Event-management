...
class OrganizedList(APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            filtered_queryset = OrganizerEventsFilter(
            ...
            return Response(
                {"error": "datas not found"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class UserProfileDetails(APIView):
    ...
    def get(self, request):
        try:
            ...
            if not username:
                return Response(
                    {"error": "Username is required."}, status=status.HTTP_404_NOT_FOUND
                )
...
class ParticipatedList(APIView):
    ...
    def get(self, request):
        try:
            ...
            event_ids = (
                Booking.objects.filter(user=user, ticket_purchases__isnull=False)
            ...
            filtered_queryset = ParticipantedEventsFilter(
        ...
        try:
            ...
            for ticket in tickets:
                ...
                refunds_data = (
                    ...
                    .annotate(date=TruncDate("refunded_at"))
                    .values("date")
                    .annotate(
                ...
                for date in all_dates:
                    purchase_data = purchases_by_date.get(
                        date, {"amount": 0, "users": 0}
                    )
                    cancellation_data = refunds_by_date.get(
                        date, {"amount": 0, "users": 0}
                    )
            ...
            total_purchases_amount = sum(
                stats["totalPurchases"] for stats in ticket_stats.values()
            )
            total_cancellations_amount = sum(
                stats["totalCancellations"] for stats in ticket_stats.values()
            )
...
class RevenueEvent(APIView):
    def get(self, request, event_id):
        try:
            ...
            return Response(
                {"error": "Revenue data for this event was not found."},
