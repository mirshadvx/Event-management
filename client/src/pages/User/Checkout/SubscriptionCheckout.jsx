...
const SubscriptionCheckout = () => {
  ...
  useEffect(() => {
    // Allow users with trial plan to purchase other plans
    // Only redirect if user has non-trial active subscription
    if (!userLoading && user && user.plan == "basic" && !user.plan_expired) {
      return;
    } else if (!user.plan) {
      return;
    } else if (user.plan === "trial" && !user.plan_expired) {
      // Allow trial plan users to purchase other plans
      return;
    ...
    const fetchPlans = async () => {
      try {
        ...
        if (response.data.success) {
          const purchasablePlans = response.data.plans.filter(
            (plan) => plan.name.toLowerCase() !== "trial"
          );
          setPlans(purchasablePlans);
          if (purchasablePlans.length > 0) {
            setSelectedPlan(purchasablePlans[0].id);
          }
        } else {
          setError(
            response.data.message || "Failed to load subscription plans"
          );
        }
      } catch (err) {
  ...
  const isTrialUser = user?.plan === "trial" && !user?.plan_expired;
        ...
        {isTrialUser && (
          <div className="bg-blue-500/20 border border-blue-500/50 text-blue-200 p-4 rounded-lg mb-6">
            <p className="font-medium">
              You're currently on a trial plan. Upgrade to a paid plan to
              continue enjoying premium features after your trial ends!
            </p>
          </div>
        )}
