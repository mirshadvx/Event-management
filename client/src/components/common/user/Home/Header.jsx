  ...
  return (
    <>
      ...
      >
          ...
          {isAuthenticated && isDesktop && (
            <>
              <nav className="flex items-center space-x-6 ml-12 bg-[#1e1e1e]/40 backdrop-blur-sm px-6 py-2 rounded-full border border-[#b4b2a8]">
                <a onClick={() => handleNavigation("/")} className="text-white/90 hover:text-white cursor-pointer">
                  Home
                </a>
                <a onClick={() => handleNavigation("/explore")} className="text-white/90 hover:text-white cursor-pointer">
                  Explore
                </a>
                <a onClick={() => handleNavigation("/profile/")} className="text-white/90 hover:text-white cursor-pointer">
                  Profile
                </a>
                <a onClick={() => handleNavigation("/dashboard")} className="text-white/90 hover:text-white cursor-pointer">
                  Dashboard
                </a>
              </nav>
              ...
              <div className="flex items-center gap-4">
                <div className="relative notification-icon">
                  <div className="relative cursor-pointer" onClick={toggleNotification}>
                    <IoIosNotifications className={`text-white w-6 h-6 ${notificationOpen ? "text-[#00FF82]" : ""}`} />
                ...
                <IoMdChatbubbles className="text-white w-6 h-6 cursor-pointer" onClick={() => handleNavigation("/chat")} />

                <div className="relative" ref={dropdownRef}>
                  <div className="flex items-center gap-2 bg-[#00FF82] px-3 py-1.5 rounded-lg cursor-pointer" onClick={toggleDropdown}>
                    <img src={user?.profile_picture || "/default-profile.png"} alt="Profile" className="w-6 h-6 rounded-full" />
                  ...
                  >
                    <ul>
                      <li onClick={() => handleNavigation("/profile/")} className="px-4 py-2 text-white/90 hover:bg-[#2e2e2e] cursor-pointer">
                        Profile
                      </li>
                      <li onClick={handleLogout} className="px-4 py-2 text-red-500 hover:bg-[#2e2e2e] cursor-pointer">
                        Logout
                      </li>
                    </ul>
                  </div>
        ...
        {isAuthenticated && !isDesktop && (
          <div ref={mobileMenuRef} className={`fixed top-0 right-0 h-full w-64 bg-[#1e1e1e] transform transition-transform duration-300 ease-in-out z-40 ${
            mobileMenuOpen ? "translate-x-0" : "translate-x-full"
          }`}>
            <div className="p-4 flex flex-col h-full">
              <div className="mt-10 mb-6 flex items-center gap-2">
                <img src={user?.profile_picture || "/default-profile.png"} alt="Profile" className="w-10 h-10 rounded-full" />
                <span className="text-white font-medium">{user?.username || "User"}</span>
              </div>

              <nav className="flex-1 space-y-4">
                <a onClick={() => handleNavigation("/")} className="text-white/90 hover:text-white text-lg block cursor-pointer">
                  Home
                </a>
                <a onClick={() => handleNavigation("/explore")} className="text-white/90 hover:text-white text-lg block cursor-pointer">
                  Explore
                </a>
                <a onClick={() => handleNavigation("/profile/")} className="text-white/90 hover:text-white text-lg block cursor-pointer">
                  Profile
                </a>
                <a onClick={() => handleNavigation("/dashboard")} className="text-white/90 hover:text-white text-lg block cursor-pointer">
                  Dashboard
                </a>
              </nav>
              ...
              <div className="mt-auto mb-6 space-y-4">
                ...
                <div className="flex items-center gap-4 cursor-pointer" onClick={() => handleNavigation("/chat")}>
                  <IoMdChatbubbles className="text-white w-6 h-6" />
                  <span className="text-white/90">Messages</span>
                </div>
                <button onClick={handleLogout} className="w-full py-2 text-red-500 hover:bg-[#2e2e2e] rounded-lg">
                  Logout
                </button>
              </div>
            </div>
