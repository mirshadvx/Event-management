...
import { ArrowUpRight, ArrowDownLeft, Wallet, Clock, History } from "lucide-react";
...
const WalletPage = () => {
    ...
    const fetchWalletData = useCallback(async (pageNum = 1, append = false) => {
        try {
            ...
            const { wallet: walletData, transactions: newTransactions, pagination } = response.data;
    ...
    const filteredTransactions = useMemo(() => {
        ...
        return transactions.filter((txn) => txn.transaction_type.toLowerCase() === activeTab.toLowerCase());
    ...
    const handleDeposit = async () => {
        ...
        try {
            ...
            const response = await api.post("/users/wallet/deposit/", { amount: parseFloat(amount) });
    ...
    const TransactionItem = ({ transaction }) => {
        const isCredit = ["DEPOSIT", "REFUND"].includes(transaction.transaction_type);
        ...
        return (
            <div className="bg-[#2d2d42] rounded-lg p-4 mb-3 flex justify-between items-center">
                <div className="flex items-center">
                    <div
                        className={`p-3 rounded-full mr-4 ${
                            isCredit ? "bg-green-900/50 text-green-400" : "bg-red-900/50 text-red-400"
                        }`}
                    >
                        {isCredit ? <ArrowDownLeft size={20} /> : <ArrowUpRight size={20} />}
                    </div>
                    <div>
                        <h4 className="text-white font-medium capitalize">{transaction.transaction_type.toLowerCase()}</h4>
                ...
                <div className={`text-right ${isCredit ? "text-green-400" : "text-red-400"}`}>
                    <div className="font-semibold flex items-center justify-end text-lg">
                        <FaRupeeSign className="mr-1" size={15} />
                        {Math.abs(transaction.amount).toFixed(2)}
                    </div>
                    <div className="text-sm text-gray-500">ID: {transaction.transaction_id.slice(0, 8)}</div>
                </div>
            </div>
        );
    };
        ...
        <div className="min-h-screen bg-[#1e1e2f] rounded-2xl p-6 shadow-xl">
            ...
            {loading && !wallet.updated_at ? (
                ...
                <div className="bg-gradient-to-r from-[#2d2d42] to-[#3a3a5a] rounded-xl p-6 mb-8 shadow-lg">
                    <div className="flex justify-between items-start">
                        <div>
                            <h2 className="text-gray-400 text-sm font-medium mb-1">Your Wallet Balance</h2>
                            <div className="flex items-center">
                            ...
                            <p className="text-gray-400 text-sm mt-2">Last updated: {formatDate(wallet.updated_at)}</p>
                        </div>
                        <div className="p-3 bg-[#00EF93]/10 rounded-full">
                            <Wallet size={32} className="text-[#00EF93]" />
                        </div>
            ...
            <div className="bg-[#2d2d42] rounded-xl p-6">
                    ...
                    <div className="flex gap-2">
                        {["all", "deposit", "payment", "refund"].map((tab) => (
                            <Button
                                ...
                                className={`${activeTab === tab ? "bg-[#00EF93] text-black" : "text-black"}`}
                                onClick={() => {
                                    setActiveTab(tab);
                                    setPage(1);
                                    fetchWalletData(1, false);
                ...
                <div className="max-h-[500px] overflow-y-auto pr-2" onScroll={handleScroll}>
                    {loading && transactions.length === 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 px-4 bg-[#2d2d42] rounded-xl">
                            <div className="text-gray-400 text-center">
                                ...
                                <h3 className="text-xl font-medium mb-1">No transactions found</h3>
                ...
                <DialogContent className="sm:max-w-[425px] bg-[#2d2d42] text-gray-200 border-gray-700">
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="flex items-center gap-4">
                            <Label htmlFor="depositAmount" className="text-right text-gray-300">
                                Amount (₹)
                            </Label>
                            <Input
                                id="depositAmount"
