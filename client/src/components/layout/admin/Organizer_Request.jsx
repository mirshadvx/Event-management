...
import { Search, CheckCircle, XCircle, RefreshCw, ChevronLeft, ChevronRight, User } from "lucide-react";
...
const OrganizerRequest = () => {
    ...
    const fetchUserDetails = async (requestId) => {
        try {
            const response = await adminApi.get(`/organizer/organizer-requests/${requestId}/user_details/`);
    ...
    const toggleAllRequests = () => {
        setSelectedRequests((prev) => (prev.length === requests.length ? [] : requests.map((r) => r.id)));
    };

    const updateRequestStatus = async (id, status, notes = "") => {
        try {
            const response = await adminApi.patch(`/organizer/organizer-requests/${id}/update_status/`, {
                status,
                admin_notes: notes,
            });
        ...
        <div className="space-y-6 p-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
            ...
            {selectedRequests.length > 0 && (
                <div className="flex flex-wrap gap-2 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    ...
                    <Button variant="outline" size="sm" onClick={() => bulkUpdateStatus("approved")} disabled={loading}>
                        <CheckCircle className="h-4 w-4 mr-2 text-green-500" />
                        Approve
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => bulkUpdateStatus("rejected")} disabled={loading}>
                        <XCircle className="h-4 w-4 mr-2 text-red-500" />
                        Reject
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => setSelectedRequests([])} disabled={loading}>
                        Clear
                    </Button>
                </div>
            )}
                            ...
                            requests.map((data) => (
                                <tr key={data.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        ...
                                        <Popover>
                                            ...
                                            <PopoverContent className="w-80">
                                                <textarea
                                                    ...
                                                    onBlur={(e) =>
                                                        updateRequestStatus(data.id, data.status, e.target.value)
                                                    }
                                                    disabled={loading}
                                                />
                                            </PopoverContent>
                                    ...
                                    <td className="p-3 text-xs text-gray-500 dark:text-gray-400">
                                        {data.requested_at ? new Date(data.requested_at).toLocaleDateString() : "N/A"}
                                    </td>
                                    <td className="p-3 text-right flex justify-end gap-2">
                                            ...
                                            <DialogContent className="w-md">
                                                ...
                                                {selectedUserDetails && (
                                                    <div className="space-y-4">
                                                        ...
                                                        <div>
                                                            <strong>Email:</strong> {selectedUserDetails.email || "N/A"}
                                                        </div>
                                                        <div>
                                                            <strong>Bio:</strong> {selectedUserDetails.bio || "None"}
                                                        </div>
                                                        <div>
                                                            <strong>Title:</strong> {selectedUserDetails.title || "None"}
                                                        </div>
                                                        <div>
                                                            <strong>Phone:</strong> {selectedUserDetails.phone || "None"}
                                                        </div>
                                                        <div>
                                                        ...
                                                        <div>
                                                            ...
                                                            {selectedUserDetails.social_media_links?.length > 0 ? (
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                <DropdownMenuItem
                                                    onClick={() => updateRequestStatus(data.id, "approved")}
                                                ...
                                                <DropdownMenuItem
                                                    onClick={() => updateRequestStatus(data.id, "rejected")}
                                                    className="flex items-center gap-2"
                                                >
