...
const SubscriptionPlansAdmin = () => {
    ...
    const handleSaveEdit = async () => {
        try {
            ...
            toast.error(`Failed to update plan: ${error.response?.data?.detail || error.message}`);
        }
    };
    ...
    const handleToggleActive = async (planId, currentStatus) => {
        try {
            ...
            console.error("Error toggling plan status:", error.response?.data || error.message);
            toast.error(`Failed to update plan status: ${error.response?.data?.detail || error.message}`);
        }
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            onChange={(e) => setPlan({ ...plan, event_join_limit: parseInt(e.target.value) || 0 })}
                            placeholder="0"
                        />
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="edit-creation-limit">Monthly Event Creation Limit</Label>
                        <Input
                            id="edit-creation-limit"
                            type="number"
                            value={plan.event_creation_limit}
                            onChange={(e) => setPlan({ ...plan, event_creation_limit: parseInt(e.target.value) || 0 })}
                            placeholder="0"
                        />
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-center space-x-2">
                        <Switch
                            id="edit-active"
                            checked={plan.active}
                            onCheckedChange={(checked) => setPlan({ ...plan, active: checked })}
                        />
                        <Label htmlFor="edit-active">{plan.active ? "Active" : "Inactive"}</Label>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {features.map((feature) => (
                            ...
                            >
                                <Label htmlFor={`edit-${feature.id}`} className="cursor-pointer">
                                    {feature.name}
                                </Label>
                                <Switch
                                    id={`edit-${feature.id}`}
                                    checked={plan[feature.id]}
                                    onCheckedChange={(checked) => setPlan({ ...plan, [feature.id]: checked })}
                                />
                            </div>
                        ))}
                    </div>
    ...
    return (
        <div className="min-h-screen text-gray-50 p-6">
            <div className="max-w-6xl mx-auto">
                {loading ? (
                    <div className="flex text-center w-full h-screen justify-center items-center"><HashLoader color="#54c955" /></div>
                ) : plans.length === 0 ? (
                    <div className="text-center">No plans available.</div>
                ) : (
                    <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 ">
                        {plans.map((plan) => (
                            <Card key={plan.id} className="bg-gray-900 border-gray-800 shadow-lg relative overflow-hidden">
                                ...
                                <CardContent>
                                    <div className="grid grid-cols-2 gap-3 mb-4">
                                        <div className="bg-gray-800 p-3 rounded-md">
                                            ...
                                            <p className="text-xl font-semibold">{plan.event_join_limit} / month</p>
                                        </div>
                                        <div className="bg-gray-800 p-3 rounded-md">
                                            <p className="text-xs text-gray-400">Create Events</p>
                                            <p className="text-xl font-semibold">{plan.event_creation_limit} / month</p>
                                        </div>
                                    </div>
                    ...
                    <Dialog open={editingPlan !== null} onOpenChange={(open) => !open && setEditingPlan(null)}>
                        <DialogContent className="bg-gray-900 text-gray-50 border-gray-800 max-w-2xl">
                            ...
                            <DialogFooter className="flex flex-row">
                                ...
                                <Button onClick={handleSaveEdit} className="bg-blue-600 hover:bg-blue-700 w-1/2">
                                    Save Changes
                                </Button>
                            </DialogFooter>
                        </DialogContent>
